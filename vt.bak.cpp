...
//std::cout treats char/wchar_t (wchar_t = short int) as, obvously, char/wchar_t and prints it as such - not it's numerical value
...
//Also, these defines only work correctly with specific std::ios::fmtflags and std::ios::fill (see below)
...
int main(int argc, char* argv[])
{
	...
	OSVERSIONINFOEX osvi_ex={sizeof(OSVERSIONINFOEX)};
	if (GetVersionEx((LPOSVERSIONINFO)&osvi_ex)||(osvi_ex.dwOSVersionInfoSize=sizeof(OSVERSIONINFO), GetVersionEx((LPOSVERSIONINFO)&osvi_ex))) {
		std::cout<<(osvi_ex.dwOSVersionInfoSize==sizeof(OSVERSIONINFOEX)?"OSVERSIONINFOEX:":"OSVERSIONINFO:")<<std::endl;
		...
		if (osvi_ex.dwOSVersionInfoSize==sizeof(OSVERSIONINFOEX)) {
			...
			SuiteMasks.Flags(osvi_ex.wSuiteMask, [](const std::string& label, DWORD value, bool unknown){
				if (unknown)
					std::cout<<"\tUNKNOWN FLAG ("<<COUT_HEX(value, 4)<<")"<<std::endl;
				else
					std::cout<<"\t"<<label<<std::endl;
			});
			std::cout<<"wProductType = "<<COUT_HEX(osvi_ex.wProductType, 4)<<std::endl;
			...
			std::cout<<"wReserved = "<<COUT_HEX(osvi_ex.wReserved, 4)<<std::endl;
		}
	} else {
